...
import ListContent from "@/pages/business/BizList/Columns";
import {listBizUsingGET} from "@/services/ant-design-pro/businessesController";

export default () => {
  const [currentRow, setCurrentRow] = useState<API.BusinessVO>();
  const [data, setData] = useState<API.BusinessVO[]>([]);
  ...
  const { loading } = useRequest(
    () => {
      const result = listBizUsingGET({ current: currentPage, pageSize: pageSize });
      return result;
    },
    {
      onSuccess: (result) => {
  ...
  function changePage(_page: number, _pageSize: number) {
    listBizUsingGET({ current: _page, pageSize: _pageSize }).then((result) => {
        ...
        >
          <List
            ...
            renderItem={(record) => (
              <List.Item
                actions={[
                  <Link to={`/business/info/${record.id}`}>
                    <a key="SUCCESS">查看</a>
                  </Link>,
                ]}
              >
