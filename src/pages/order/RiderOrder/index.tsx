...
import {listRiderMyOrderUsingGET} from "@/services/ant-design-pro/ridersController";
...
export default () => {
  ...
  const { loading } = useRequest(
    () => {
      const result = listRiderMyOrderUsingGET({ riderId: initialState.riderId, current: currentPage, pageSize: pageSize });
      return result;
    },
    {
      onSuccess: (result) => {
  ...
  function changePage(_page: number, _pageSize: number) {
    listRiderMyOrderUsingGET({ riderId: initialState.riderId, current: _page, pageSize: _pageSize }).then((result) => {
