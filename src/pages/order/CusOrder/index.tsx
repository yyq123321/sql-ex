...
import {listCusMyOrderUsingGET} from "@/services/ant-design-pro/customersController";
...
export default () => {
  ...
  const { loading } = useRequest(
    () => {
      const result = listCusMyOrderUsingGET({ customerId: initialState.customerId, current: currentPage, pageSize: pageSize });
      return result;
    },
    {
      onSuccess: (result) => {
  ...
  function changePage(_page: number, _pageSize: number) {
    listCusMyOrderUsingGET({ customerId: initialState.customerId, current: _page, pageSize: _pageSize }).then((result) => {
