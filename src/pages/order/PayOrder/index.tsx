...
import { createOrderUsingPOST, getOrderUsingGET } from '@/services/ant-design-pro/orderController';
...
const PayOrder: React.FC = () => {
  ...
  const queryOrderStatus = async () => {
    ...
    return await getOrderUsingGET({ orderSn: order?.orderNumber });
  };
  ...
  const toAlipay = async () => {
    ...
    setLoading(true);
    const res = await createOrderUsingPOST({
      productId: params.id,
  ...
  useEffect(() => {
    if (order && order.orderNumber && order.codeUrl) {
      const intervalId = setInterval(async () => {
        ...
        if (res.data && res.code === 0) {
          ...
          setTimeout(function () {
            ...
            history.push(urlParam.get('redirect') || '/account/center');
          }, 2000);
        } else {
          console.log('支付中...');
        }
          ...
          <ProCard bordered headerBordered layout={'center'} title={<strong>支付方式</strong>}>
            <Radio.Group name="payType" value={payType}>
              <ProCard wrap gutter={18}>
                ...
                >
                  <Radio disabled={true} value={'WX'} style={{ fontSize: '1.12rem' }}>
                    <WxPay /> 微信支付
                  </Radio>
                </ProCard>
                <ProCard
