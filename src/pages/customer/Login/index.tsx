...
import {getLoginUserUsingGET1, loginUsingPOST1} from "@/services/ant-design-pro/customersController";
...
const Login: React.FC = () => {
  ...
  const fetchUserInfo = async () => {
    const userInfo = await getLoginUserUsingGET1();
    if (userInfo) {
  ...
  const handleSubmit = async (values: API.CustomerLoginRequest) => {
    try {
      // 登录
      const msg = await loginUsingPOST1({
        ...values,
      });
      if (msg.data) {
        console.log(msg);
        ...
        history.push('/cusIndex' || redirect);
          ...
          <div className={styles.content}>
            <LoginForm
              ...
              onFinish={async (values) => {
                await handleSubmit(values as API.CustomerLoginRequest);
              }}
              form={form}
            >
