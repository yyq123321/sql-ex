...
const Projects: FC = () => {
  ...
  const { loading } = useRequest(
    () => {
      const result = queryCourseLikeUsingGET({
        name: '',
        current: current,
        pageSize: pageSize,
        status: 1,
  ...
  function changePage(_page: number, _pageSize: number) {
    queryCourseLikeUsingGET({ name: name, current: _page, pageSize: _pageSize }).then((result) => {
  ...
  const handleFormSubmit = async (value: string) => {
    setName(value);
    const result = await queryCourseLikeUsingGET({
      name: value,
      current: current,
      pageSize: pageSize,
    });
                ...
                title={
                  ...
                  >
                    ...
                    <div className={styles.avatarList}>
                      ...
                      <Avatar size="small" className={styles.avatar} src={item.logo} alt="avatar" />
                    </div>
                  </div>
                }
                description={
